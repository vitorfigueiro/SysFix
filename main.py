...
class BackupManager:
    """Gerencia a geração de backups do banco de dados SQLite"""
    @staticmethod
    def criar_backup(destino_dir) -> str:
        ...
        nome_zip = f"backup_coletas_{timestamp}.zip"
...
class Application(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Sistema de Gestão e Controle de Equipamentos (VM Server)")
        ...
        # Intercepta o fechamento para backup automático
    ...
    def setup_styles(self):
        ...
        style.configure("TLabelframe", background=self.colors["bg_card"], relief="solid", borderwidth=1, bordercolor=self.colors["border"])
        style.configure("TLabelframe.Label", font=("Segoe UI", 10, "bold"), foreground=self.colors["primary"], background=self.colors["bg_card"])

        style.configure("TLabel", background=self.colors["bg_card"], foreground=self.colors["text_main"])
        style.configure("TEntry", fieldbackground="#FFFFFF", foreground=self.colors["text_main"], bordercolor=self.colors["border"])
        style.map("TEntry", bordercolor=[("focus", self.colors["primary"])])

        style.configure("TButton", font=("Segoe UI", 9, "bold"), padding=7, relief="flat", borderwidth=0, background="#E2E8F0", foreground=self.colors["text_main"])
        ...
        style.configure("TNotebook.Tab", font=("Segoe UI", 9, "bold"), padding=[14, 7], background="#94A3B8", foreground="#FFFFFF")
        style.map("TNotebook.Tab", 
                  background=[("selected", self.colors["primary"])], 
                  foreground=[("selected", "#FFFFFF")])

    def autenticar_admin(self) -> bool:
        senha = simpledialog.askstring("Autenticação de Responsável", 
    ...
    def create_widgets(self):
        ...
        ttk.Label(tab_entrada, text="Localização:", font=("Segoe UI", 9, "bold")).grid(row=5, column=0, sticky="w", pady=4)
        ...
        ttk.Label(tab_entrada, text="Problema:", font=("Segoe UI", 9, "bold")).grid(row=6, column=0, sticky="nw", pady=4)
        self.text_problema = tk.Text(tab_entrada, width=20, height=3, font=("Segoe UI", 9), relief="solid", bd=1, highlightbackground=self.colors["border"])
        self.text_problema.grid(row=6, column=1, pady=4, sticky="ew")

        ttk.Button(tab_entrada, text="➕ Registrar Entrada", style="Primary.TButton", command=self.salvar_entrada).grid(row=7, column=0, columnspan=2, pady=(12, 4), sticky="ew")
        ttk.Button(tab_entrada, text="🔒 ✏️ Editar Dados Entrada (Resp.)", command=self.atualizar_entrada).grid(row=8, column=0, columnspan=2, pady=2, sticky="ew")

        # --- ABA SAÍDA ---
        tab_saida = ttk.Frame(self.notebook, style="Card.TFrame", padding=12)
        self.notebook.add(tab_saida, text="📤 Saída")

        self.lbl_item_selecionado = ttk.Label(tab_saida, text="⚠️ Nenhum item selecionado", font=("Segoe UI", 9, "bold"), foreground=self.colors["danger"])
        ...
        ttk.Label(tab_saida, text="Laudado?", font=("Segoe UI", 9, "bold")).grid(row=2, column=0, sticky="w", pady=4)
        ...
        ttk.Label(tab_saida, text="Status Custo:", font=("Segoe UI", 9, "bold")).grid(row=3, column=0, sticky="w", pady=4)
        ...
        ttk.Label(tab_saida, text="Valor Custo (R$):", font=("Segoe UI", 9, "bold")).grid(row=4, column=0, sticky="w", pady=4)
        ...
        ttk.Label(tab_saida, text="Resolução:", font=("Segoe UI", 9, "bold")).grid(row=5, column=0, sticky="nw", pady=4)
        self.text_resolucao = tk.Text(tab_saida, width=20, height=3, font=("Segoe UI", 9), relief="solid", bd=1, highlightbackground=self.colors["border"])
        self.text_resolucao.grid(row=5, column=1, pady=4, sticky="ew")

        ttk.Button(tab_saida, text="✅ Finalizar & Marcar Entregue", style="Primary.TButton", command=self.salvar_saida).grid(row=6, column=0, columnspan=2, pady=12, sticky="ew")

        # Botões de Ação Inferiores
        frame_acoes = ttk.Frame(left_panel, padding=(0, 6, 0, 0))
        frame_acoes.pack(fill="x")
        ttk.Button(frame_acoes, text="🔒 🗑️ Excluir (Resp.)", style="Danger.TButton", command=self.excluir_registro).pack(side="left", expand=True, fill="x", padx=(0, 2))
        ttk.Button(frame_acoes, text="🧹 Limpar", command=self.limpar_formularios).pack(side="left", expand=True, fill="x", padx=(2, 0))

        # --- SEÇÃO DE RELATÓRIOS E BACKUP ---
        frame_reports = ttk.LabelFrame(left_panel, text=" Exportação & Segurança ", padding=8)
        frame_reports.pack(fill="x", pady=(6, 0))
        
        ttk.Button(frame_reports, text="📄 Gerar Relatório Mensal (PDF)", style="Primary.TButton", command=self.abrir_janela_relatorio).pack(fill="x", pady=(0, 4))
        ttk.Button(frame_reports, text="💾 Executar Backup Agora (ZIP)", command=self.executar_backup_manual).pack(fill="x", pady=2)
        ttk.Button(frame_reports, text="⚙️ Selecionar Pasta de Backup", command=self.configurar_pasta_backup).pack(fill="x", pady=2)
        ...
        ttk.Button(frame_filtros, text="⏳ Pendentes", command=lambda: self.filtrar_tabela("nao_finalizados_mes")).pack(side="left", expand=True, fill="x", padx=1)
        ttk.Button(frame_filtros, text="✅ Finalizados", command=lambda: self.filtrar_tabela("finalizados_mes")).pack(side="left", expand=True, fill="x", padx=1)
        ttk.Button(frame_filtros, text="📋 Todos", command=lambda: self.filtrar_tabela("todos")).pack(side="left", expand=True, fill="x", padx=1)
    ...
    def executar_backup_manual(self):
        if not self.backup_dir:
            messagebox.showwarning("Aviso", "Por favor, defina primeiro a pasta onde os backups serão salvos no botão abaixo.")
            return

        try:
            zip_criado = BackupManager.criar_backup(self.backup_dir)
            messagebox.showinfo("Backup Concluído", f"Backup gerado com sucesso:\n{zip_criado}")
        except Exception as e:
            messagebox.showerror("Erro no Backup", f"Ocorreu uma falha ao gerar o backup: {str(e)}")

    def on_closing(self):
        """Ação ao fechar a aplicação (realiza backup automático se configurado)"""
        if self.backup_dir and os.path.exists(self.backup_dir):
            try:
                BackupManager.criar_backup(self.backup_dir)
            except Exception as e:
                print(f"Erro no backup de fechamento: {e}")
        self.destroy()
    ...
    def _add_field(self, parent, row, icon_label, value_text, is_full_width=False):
        ...
        val = tk.Label(parent, text=value_text, font=("Segoe UI", 9, "bold"), fg=self.colors["text_main"], bg="#FFFFFF", anchor="w", justify="left", wraplength=350 if is_full_width else 220)
    ...
    def renderizar_ficha(self, reg):
        ...
        data_br = datetime.strptime(reg[4], "%Y-%m-%d").strftime("%d/%m/%Y")
        val_fmt = f"R$ {reg[10]:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
        status = reg[8]
        ...
        lbl_id = tk.Label(card_header, text=f"REGISTRO #{reg[0]}", font=("Segoe UI", 9, "bold"), fg=self.colors["secondary"], bg="#FFFFFF")
        lbl_id.pack(anchor="w")

        lbl_titulo = tk.Label(card_header, text=str(reg[1]).upper(), font=("Segoe UI", 13, "bold"), fg=self.colors["primary"], bg="#FFFFFF")
        ...
        self._add_field(card_tech, 1, "🏷️ Tombamento:", reg[2])
        self._add_field(card_tech, 2, "📍 Localização:", reg[6])
        ...
        self._add_field(card_entrada, 1, "👤 Técnico Coleta:", reg[3])
        self._add_field(card_entrada, 2, "📅 Data da Coleta:", data_br)
        self._add_field(card_entrada, 3, "🏢 Unidade Origem:", reg[5])
        self._add_field(card_entrada, 4, "⚠️ Problema Relatado:", reg[7] if reg[7] else "Nenhum defeito relatado", is_full_width=True)
        ...
        self._add_field(card_saida, 1, "📄 Laudo Técnico:", reg[13] if reg[13] else "Não")
        self._add_field(card_saida, 2, "👤 Téc. Resp. Entrega:", reg[12] if reg[12] else "Pendente")
        self._add_field(card_saida, 3, "💰 Status de Custo:", f"{reg[9]} ({val_fmt})")
        self._add_field(card_saida, 4, "🛠️ Serviço / Resolução:", reg[11] if reg[11] else "Em andamento / Aguardando manutenção", is_full_width=True)
    ...
    def on_item_select(self, event):
        ...
        reg = next((r for r in self.todos_registros_cache if r[0] == self.registro_selecionado_id), None)
        if not reg:
            return
        ...
        self.entry_equip.delete(0, tk.END); self.entry_equip.insert(0, reg[1])
        self.entry_tombamento.delete(0, tk.END); self.entry_tombamento.insert(0, reg[2])
        self.entry_tecnico.delete(0, tk.END); self.entry_tecnico.insert(0, reg[3])
        self.entry_data.delete(0, tk.END); self.entry_data.insert(0, reg[4])
        self.entry_origem.delete(0, tk.END); self.entry_origem.insert(0, reg[5])
        ...
        if reg[7]: self.text_problema.insert("1.0", str(reg[7]))

        if reg[13] in ["Não", "Sim"]:
            self.combo_laudado.set(reg[13])
            
        st_custo = reg[9] if reg[9] in ["Sem Custo", "Com Custo"] else "Sem Custo"
        ...
        if reg[11]: self.text_resolucao.insert("1.0", str(reg[11]))

        self.entry_tecnico_entrega.delete(0, tk.END)
        if reg[12]: self.entry_tecnico_entrega.insert(0, str(reg[12]))
    ...
    def carregar_dados_tabela(self):
        ...
        if self.filtro_ativo == "nao_finalizados_mes":
            self.todos_registros_cache = ColetaModel.buscar_nao_finalizados_mes_atual()
        elif self.filtro_ativo == "finalizados_mes":
            self.todos_registros_cache = ColetaModel.buscar_finalizados_mes_atual()
        else:
            self.todos_registros_cache = ColetaModel.buscar_todos()

        for reg in self.todos_registros_cache:
            self.tree.insert("", "end", values=(reg[0], reg[1], reg[2], reg[8]))
    ...
    def salvar_entrada(self):
        try:
            registro_id = ColetaModel.registrar_entrada(
                self.entry_equip.get(), self.entry_tombamento.get(), self.entry_tecnico.get(),
                self.entry_data.get(), self.entry_origem.get(), self.combo_localizacao.get(),
    ...
    def atualizar_entrada(self):
        ...
        try:
            ...
            messagebox.showinfo("Sucesso", f"Registro ID #{self.registro_selecionado_id} atualizado com sucesso!")
            self.carregar_dados_tabela()
        except Exception as e:
    ...
    def salvar_saida(self):
        if not self.registro_selecionado_id:
            messagebox.showwarning("Aviso", "Selecione um equipamento na lista para registrar a saída.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Sucesso", f"Equipamento ID #{self.registro_selecionado_id} marcado como ENTREGUE!")
            self.limpar_formularios()
            self.carregar_dados_tabela()
        except Exception as e:
    ...
    def excluir_registro(self):
        ...
        if messagebox.askyesno("Confirmar Exclusão", f"Tem certeza que deseja excluir o registro ID #{self.registro_selecionado_id}?"):
    ...
    def abrir_janela_relatorio(self):
        ...
        ttk.Label(win, text="Selecione o Mês e Ano:", font=("Segoe UI", 10, "bold")).pack(pady=15)

        frame_sel = ttk.Frame(win, style="Card.TFrame")
        frame_sel.pack(pady=5)

        meses = [f"{i:02d}" for i in range(1, 13)]
        cb_mes = ttk.Combobox(frame_sel, values=meses, width=5, state="readonly")
        cb_mes.set(datetime.now().strftime("%m"))
        cb_mes.pack(side="left", padx=5)

        anos = [str(a) for a in range(2024, datetime.now().year + 2)]
        cb_ano = ttk.Combobox(frame_sel, values=anos, width=8, state="readonly")
        ...
        def gerar():
            m = int(cb_mes.get())
            a = int(cb_ano.get())
            folder = filedialog.askdirectory(title="Selecione onde salvar o PDF")
            if folder:
                try:
                    caminho = PDFReportGenerator.relatorio_por_mes(folder, m, a)
                    messagebox.showinfo("Sucesso", f"Relatório gerado com sucesso!\n{caminho}")
                    win.destroy()
                except Exception as e:
                    messagebox.showerror("Erro", str(e))

        ttk.Button(win, text="📥 Gerar e Salvar PDF", style="Primary.TButton", command=gerar).pack(pady=15)

if __name__ == "__main__":
    # 1. Tenta atualizar o programa antes de abrir
    verificar_e_atualizar()

    # 2. Roda as migrações no banco sem apagar/substituir o arquivo coletas.db
    init_db()
